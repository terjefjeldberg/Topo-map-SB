...
window.searchEpsgSetup = function(val) {
  ...
  if (!results.length) { dd.innerHTML = '<div style="padding:6px 10px;font-family:DM Mono,monospace;font-size:11px;color:#555">Ingen treff</div>'; return; }
  dd.innerHTML = results.map(function(r) {
    return '<div class="epsg-setup-item" data-code="' + r.code + '" data-name="' + r.name.replace(/"/g,'&quot;') + '" onclick="selectEpsgSetup(this.dataset.code,this.dataset.name)">' +
      '<span class="epsg-code">' + r.code + '</span>' +
      '<span class="epsg-name">' + r.name + '</span></div>';
  }).join('');
};
...
window.searchEpsg = function(val) {
  ...
  var results = epsgSearch(val);
  if (!results.length) { dd.innerHTML = '<div style="padding:6px 10px;font-family:DM Mono,monospace;font-size:11px;color:#555">Ingen treff</div>'; dd.classList.add('open'); return; }
  dd.innerHTML = results.map(function(r) {
    return '<div class="epsg-item" data-code="' + r.code + '" data-name="' + r.name.replace(/"/g,'&quot;') + '" onclick="selectEpsgTopbar(this.dataset.code,this.dataset.name)">' +
      '<span class="epsg-code">' + r.code + '</span>' +
      '<span class="epsg-name">' + r.name + '</span></div>';
  }).join('');
